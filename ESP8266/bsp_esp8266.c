...
{    
	...
	//***当进行查询外部WIFI命令时，对反馈信息进行过滤处理***///
	if(strcmp(cmd, "AT+CWLAP") == 0){
		
		printf ( "\r\n\r\n [Mode]AT+CWLAP \r\n" ); 
		
		//分割反馈信息，将分割后的若干字符串存入一个字符串数组内
		p = strtok(strEsp8266_Fram_Record .Data_RX_BUF, split);
		while(p != NULL){
			list1[val1++] = p;
			p = strtok(NULL, split);
		}
		...
		//WIFI名称去重，扫描WIFI时可能重复获得同一个WIFI的信息
		while(list2[val2]){
			while(list1[val3]){
				if(strcmp(list1[val3++], list2[val2]) == 0){
					flag = 1;
					break;
				}
			}
